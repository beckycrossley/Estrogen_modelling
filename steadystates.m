function F = steadystates(x)

% Unknown variables
C_in = x(1);
C_SR = x(2);

% -----------------------------
% Parameters
% -----------------------------
p       = 0;

k_IPR = 5.55;
V_p     = 4.5;
k_p     = 0.4;
n       = 4;

a_0     = 0.05;
a_1     = 0.25;
a_2     = 1.0;

V_e     = 4.5;
k_e     = 0.1;

%k_IP3R  = 5.55;
k_RYR   = 5.0;

J_er    = 0.1;

gCa   = 9.0;
delta = 0.05;
gamma = 5.5;
V_m     = -50;
k_m     = 12;

V       = -60;

Fconst  = 96485.3329;
R       = 8314;
T       = 310;
C_0     = 1.23;
K_1     = 0.13;
K_5     = 0.082;
%K_3     = 0.943;
y       = 0.1;

% -----------------------------
% Auxiliary variables
% -----------------------------

m = 1/(1 + exp(-(V - V_m)/k_m));

exp_term = exp(-2*V*Fconst/(R*T));

V_Ca = V*(C_in - C_0*exp_term)/(1 - exp_term);

I_Ca = gCa*m^2*V_Ca;

P_RYR = C_in^n/(k_p^n + C_in^n);

P_IPR = ((p*C_in*(1-y))/(p + K_1)*(C_in + K_5))^3;

%P_IP3R = (p*C_in*(1-y)/((p + K_1)*(C_in + K_5)))^3;

% -----------------------------
% Fluxes
% -----------------------------
J_release = (k_IPR*P_IPR + k_RYR* P_RYR + J_er)*(C_SR -C_in);

%J_PMCA = V_p*C_in^n/(k_p^n + C_in^n);

J_in = a_0 - a_1*I_Ca/(2*Fconst) + a_2*p;

J_SERCA = V_e*C_in^2/(k_e^2 + C_in^2);

J_pm = V_p*C_in^4/(k_p^4 + C_in^4);

%J_IP3R = k_IP3R*P_IP3R*(C_SR - C_in);

%J_RYR = k_RYR*P_RYR*(C_SR - C_in);

%J_leak = J_er*(C_SR - C_in);

% -----------------------------
% Steady-state equations
% -----------------------------

F = zeros(2,1);

F(1) = J_release -J_SERCA + delta*(J_in -J_pm);

F(2) = gamma*(J_SERCA - J_release);


end
